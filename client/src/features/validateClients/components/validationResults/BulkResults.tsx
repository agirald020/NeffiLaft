...
const BulkResults: FunctionComponent<BulkResultsProps> = () => {
  // selector puntual para evitar rerenders innecesarios
  ...
  // tu tabla original adaptada al tipado actual
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 text-center">
            <p className="text-2xl font-bold text-gray-900 dark:text-white" data-testid="text-bulk-total">
              {stats.total}
            </p>
      ...
      {bulkResults.map((result, idx) => (
        <div
          key={idx}
          className={`bg-white dark:bg-gray-900 rounded-2xl shadow-sm border overflow-hidden ${result.matchCount > 0 ? "border-red-200 dark:border-red-900" : "border-gray-200 dark:border-gray-800"}`}
          data-testid={`bulk-result-${idx}`}
        >
          <div className={`px-6 py-3 flex items-center justify-between ${result.matchCount > 0 ? "bg-red-50 dark:bg-red-950/30" : "bg-green-50 dark:bg-green-950/30"}`}>
            <div className="flex items-center space-x-3">
              {result.matchCount > 0 ? <AlertTriangle className="w-4 h-4 text-red-500" /> : <CheckCircle2 className="w-4 h-4 text-green-500" />}
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">
                  {result.queryDocumentNumber || result.queryFullName || "—"}
                  {result.queryFullName && result.queryDocumentNumber && <span className="text-gray-400 font-normal"> — {result.queryFullName}</span>}
                </p>
              </div>
            </div>
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${result.matchCount > 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
